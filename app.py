...
# Configuration
# --------------------------------------------------------------
BASE_DIR = os.path.abspath(os.path.dirname(__file__))          # ✅ fixed
...
# --- Fixed SECRET_KEY and CSRF disabled ---
...
class Department(db.Model):
    __tablename__ = "departments"               # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class Category(db.Model):
    __tablename__ = "categories"                # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(80), unique=True, nullable=False)
    description = db.Column(db.Text)

class WorkingItem(db.Model):
    __tablename__ = "working_items"             # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(80), unique=True, nullable=False)
    description = db.Column(db.Text)

class Employee(db.Model):
    __tablename__ = "employees"                 # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class MaintenanceRequest(db.Model):
    __tablename__ = "maintenance_requests"      # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class WorkOrder(db.Model):
    __tablename__ = "work_orders"               # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class WorkOrderPart(db.Model):
    __tablename__ = "work_order_parts"          # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class InventoryPart(db.Model):
    __tablename__ = "inventory_parts"           # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class StockMovement(db.Model):
    __tablename__ = "stock_movements"           # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class Contractor(db.Model):
    __tablename__ = "contractors"               # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), nullable=False)
...
class PreventiveMaintenance(db.Model):
    __tablename__ = "preventive_maintenance"    # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class ChecklistTemplate(db.Model):
    __tablename__ = "checklist_templates"       # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), unique=True, nullable=False)
    description = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class ChecklistTemplateItem(db.Model):
    __tablename__ = "checklist_template_items"  # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class Inspection(db.Model):
    __tablename__ = "inspections"               # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class InspectionItem(db.Model):
    __tablename__ = "inspection_items"          # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class Notification(db.Model):
    __tablename__ = "notifications"             # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
...
class AuditLog(db.Model):
    __tablename__ = "audit_logs"                # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"))
...
class StatusHistory(db.Model):
    __tablename__ = "status_history"            # ✅ fixed
    id = db.Column(db.Integer, primary_key=True)
    request_id = db.Column(db.Integer, db.ForeignKey("maintenance_requests.id"))
...
# --------------------------------------------------------------
# ROUTES (CSRF validation removed)
# --------------------------------------------------------------
@app.route("/")
def index():
...
# ---------- Additional Routes for Navigation Menu ----------
@app.route('/work_orders')
def work_orders():
    try:
        return render_template('work_orders.html', work_orders=[])
    except Exception:
        return render_template('dashboard.html')
...
def areas():
    try:
        return render_template('areas.html', areas=[])
    except Exception:
        return render_template('dashboard.html')
...
def preventive():
    try:
        return render_template('preventive.html', preventive=[])
    except Exception:
        return render_template('dashboard.html')
...
def checklists():
    try:
        return render_template('checklists.html', checklists=[])
    except Exception:
        return render_template('dashboard.html')
